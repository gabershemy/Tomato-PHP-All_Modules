<x-tomato-admin-container label="{{trans('tomato-admin::global.crud.edit')}} {{__('Inventory')}} #{{$model->id}}">
    <x-splade-form class="flex flex-col space-y-4" action="{{route('admin.inventories.update', $model->id)}}" method="post" :default="$model">

        <x-splade-select :label="__('From Company')" :placeholder="__('From Company')" name="company_id" remote-url="/admin/companies/api" remote-root="data" option-label="name" option-value="id" choices/>
        ...
        <x-splade-checkbox :label="__('Is transaction')" name="is_transaction" label="Is transaction" />
        <x-splade-select v-if="form.is_transaction" :label="__('Transaction to branch')" :placeholder="__('Transaction to branch')" name="to_branch_id" remote-url="/admin/branches/api" remote-root="data" option-label="name" option-value="id" choices/>
        <x-splade-select v-if="!form.is_transaction" choices :label="__('Type')" name="type" type="text"  :placeholder="__('Type')">
