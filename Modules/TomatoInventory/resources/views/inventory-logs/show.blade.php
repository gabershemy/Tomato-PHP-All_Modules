<x-tomato-admin-container label="{{trans('tomato-admin::global.crud.view')}} {{__('inventory-logs')}} #{{$model->id}}">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        
          <x-tomato-admin-row :label="__('User')" :value="$model->User->name" type="text" />

          <x-tomato-admin-row :label="__('Inventory')" :value="$model->Inventory->id" type="text" />
    ...
    <div class="flex justify-start gap-2 pt-3">
        <x-tomato-admin-button warning label="{{__('Edit')}}" :href="route('admin.inventory-logs.edit', $model->id)"/>
