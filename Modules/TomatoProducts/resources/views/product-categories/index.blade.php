<x-tomato-admin-container label="{{__('Product Categories')}}">
    <x-slot:buttons>
        <x-tomato-admin-button warning type="link" :modal="true" :href="route('admin.products.index')">
            {{__('Back')}}
        </x-tomato-admin-button>
        <x-tomato-admin-button type="link" :modal="true" :href="route('admin.products.category.create')">
            {{__('Add Category')}}
        </x-tomato-admin-button>
    </x-slot:buttons>
            ...
            <x-splade-table :for="(new \Modules\TomatoCategory\App\Tables\CategoryTable(\Modules\TomatoCategory\App\Models\Category::where('for', 'product-categories')))">
                ...
                <x-splade-cell actions>
                    <div class="flex justify-start">
                        <div class="flex justify-start">
                            <x-tomato-admin-button type="icon" warning modal title="{{trans('tomato-admin::global.crud.edit')}}" href="{{ route('admin.products.category.edit', $item->id) }}">
                                <x-heroicon-s-pencil class="h-6 w-6"/>
                            </x-tomato-admin-button>

                            <x-tomato-admin-button type="icon" title="{{trans('tomato-admin::global.crud.delete')}}" href="{{ route('admin.products.category.delete',  $item->id) }}"
